...
first_node = Node("Salut !\nJe suis votre Great-Helper, HETIC m'a engagé afin d'apporter de l'aide et répondre à ses (nouveaux) étudiants en développement WEB.\nÀ tout moment tu peut écrire 'return' pour revenir à la question précédente, 'reset' pour redémarrer la conversation, 'fin' pour l'arrêter ou'$info' pour comprendre mon fonctionnement.\nDit-moi comment je peut t'aider ? (cours, alternance, stage, administration)","great-helper",first_list)
lescommandes = ['great-helper!','$info','$add','$modo','$heure','$date','$clear','$date','fin']
...
async def on_message(message):
    ...
    if message.author == client.user:
        return
    Help_channel = client.get_channel(978276027329564692)
    global current_node
    global history
    global mode_help
    ...
    if message.channel == Help_channel and message.content =='great-helper!':
        mode_help = True
...
client.run("OTc4MjMwNDQwOTUzNTQwNjgw.GD68P6.0Zc5qeWzvERtptXNNDwag0JCfywHJ6ePUemido")